...
const fs = require('fs');
const configPath = './configs/civiConfig.json';
...
module.exports = {
    name: 'setcivi',
    description: 'Ορίζει το κανάλι για την ενημέρωση των Civilian Jobs.',
    async run(client, message, args) {
        ...
        updateCiviEmbed(client, config);

        if (refreshInterval) clearInterval(refreshInterval);
        refreshInterval = setInterval(() => updateCiviEmbed(client, config), 60000);
    }
};

async function updateCiviEmbed(client, config) {
    ...
    const embed = new MessageEmbed()
        .setTitle('Civilian Jobs Members')
        .setColor('#00ff00')
